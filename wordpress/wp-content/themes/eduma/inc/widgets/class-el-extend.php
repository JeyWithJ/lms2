...
class Thim_Elementor_Extend {
	...
	public function __construct() {
		...
		add_action( 'elementor/widgets/widgets_registered', array( $this, 'thim_register_widgets' ), 200 );
	...
	public function thim_register_widgets( $widgets_manager ) {
			...
			foreach ( $widgets_all as $base => $widgets ) {
				...
				if ( $base == 'general' || ( $base != 'general' && class_exists( $base ) ) ) {
					foreach ( $widgets as $widget ) {
						...
						$widgets_manager->unregister_widget_type( 'wp-widget-' . $widget );
		...
		if ( class_exists( '\Elementor\Thim_Ekit_Widget_Login_Popup' ) ) {
			$widgets_manager->register_widget_type( new \Elementor\Thim_Ekit_Widget_Login_Popup() );
		}
	}
