	...
	private function buyCourse() {
		try {
			...
			&& isset( $_POST['purchase-course'] ) ) {
				...
				if ( ! empty( $_POST['purchase-course'] ) ) {
					$course_id   = LP_Request::get_param( 'purchase-course', 0, 'int' );
					$is_purchase = true;
				} elseif ( ! empty( $_POST['enroll-course'] ) ) {
					$course_id = LP_Request::get_param( 'enroll-course', 0, 'int' );
				}

				$lp_rest = new LP_REST_Courses_Controller();
				$request = new WP_REST_Request( 'POST' );
				$request->set_param( 'id', $course_id );
				if ( $is_purchase ) {
					$res = $lp_rest->purchase_course( $request );

					if ( $res->get_status() == 200 && isset( $res->get_data()->status )
						&& $res->get_data()->status === 'success' ) {
						$redirect_to = $res->get_data()->data->redirect;
						wp_safe_redirect( $redirect_to );
						die;
					}
				} else {
					...
					$res    = wp_remote_post( home_url( 'wp-json/lp/v1/courses/enroll-course' ), $params );
